...
def init_db():    
    ...
    if lon<=0:
        ...
        cursor.execute(Query)
        Query="CREATE TABLE VEHICLES (ID_VEHICLE INT PRIMARY KEY AUTO_INCREMENT,Numero_Placa VARCHAR(255) NOT NULL,Marca VARCHAR(255) NOT NULL, Modelo VARCHAR(255) NOT NULL, Tipo_Combustible VARCHAR(255) NOT NULL, Capacidad_Tanque VARCHAR(255) NOT NULL, Poliza VARCHAR(255) NOT NULL,Vencimiento_Poliza VARCHAR(255) NOT NULL,Codigo_Flota VARCHAR(255) NOT NULL,Fecha_Registro DATETIME DEFAULT CURRENT_TIMESTAMP);"
        cursor.execute(Query)
